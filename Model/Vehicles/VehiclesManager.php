...
class VehiclesManager {
    ...
    function insert_vehicles($autoParkData) {
        ...
        if ($reqt == NULL) {
            $q = $this->_db->prepare("INSERT INTO vehicles SET idModel = :idModel, matriculation = :matriculation, seats = :seats, colour = :colour, picture = :picture, technology = :technology, simNumber = :simNumber, feeWithDriver = :feeWithDriver, feeWithoutDriver = :feeWithoutDriver ") or die(mysql_error());
            ...
            $q->bindValue(':technology', $autoParkData->getTechnology());
...
//                window.location.href = String(reload);
    ...
    public function update_vehicle($autoParkData) {
            $q = $this->_db->prepare("UPDATE vehicles SET idModel = :idModel, matriculation = :matriculation, seats = :seats, colour = :colour, picture = :picture, technology = :technology, simNumber = :simNumber, feeWithDriver = :feeWithDriver, feeWithoutDriver = :feeWithoutDriver WHERE idVehicle = :idVehicle") or die(mysql_error());
